...
> Rule-set version this project records: **0.37.0** (D15 adoption 2026-09-25: checkout `1acd773` tag `contract-v0.37.0` published, one step from 0.36.0 (D1); delta = packet `coder:`/`model:`/`effort:` retired → refused `packet_settings_retired` (D302: settings move to the trigger, precedence command>lane conf>Coder default, Launcher defaults incl opencode `max`, relaunch fallback `coder_default`) + CLOSE trim wording (D299: LIVE items stay, trigger-waiting items move to backlog) + catalogue refresh; installed `18de7fd7…` = payload `RULES.sha256` at both versions — clean, was behind; supersedes `0.33.0`).
...
| **Coder** | `opencode` (SELECTED from `CODERS.md` — `opencode run --auto --dir <workdir> [--variant <effort>] <prompt-text>`, prompt travels on argv) | Owner directive `D29` (2026-09-11) — opencode going forward; supersedes `m0106` (codex high, kept as history). `G-O2` — packet says "the Coder", config names it. Effort `low|high|max` for opencode (lane refuses `medium`/`xhigh` by name) → `--variant` (explicit `high` on every trigger per D120/D15, uncalibrated per `G-A9`; the Coder default `max` is NOT taken — stated explicitly so no silent drift). Catalogue `coder_models.tsv` (0.37.0): `high` listed for `opencode-go/deepseek-v4.1-flash` — engine-checked strictly, trigger effort (D266/D302). Hard ceiling: packet file stays far under the 128 KiB single-argv limit. Provider partially unproven (`ISS-6`); entry-point run-2 owed (`ISS-7`) |
| **Dispatch verb** | `SG-<nnn> [coder= model= effort=] [--status\|--attach\|--force]` (e.g. `SG-021 coder=opencode effort=high`) | ID prefix `SG-` for StorageGenie. Settings ride the trigger, any subset (D302) — the packet carries the task only; a packet `coder:`/`model:`/`effort:` line is refused `packet_settings_retired`. `--force` bypasses replay guard only, packet resolves `$ID.md\|$ID-*.md` exactly-one |
...
- Session start: `G-L1` version check FIRST (fetch `origin contract` in `.rules-cache/`, read `FETCH_HEAD:VERSION`, and hash the installed global rules file against the payload `RULES.sha256` — never checkout-vs-stamp, M3), then this file + `.rules-cache/` (contract `contract-v0.37.0`) + `STATE.md:1` RESUME.
