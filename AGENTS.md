...
| **Coder** | `codex` (SELECTED from `global/CODERS.md` — `codex exec --sandbox danger-full-access [-m <model>] [-c model_reasoning_effort=<effort>] -C <workdir> < <prompt-file>`) | `G-O2` — packet says "the Coder", config names it. `opencode` gated by `~/.local/share/opencode` defect (`vps-info.md:172`). Allowed effort `low|medium|high|xhigh` (`medium` default) |
