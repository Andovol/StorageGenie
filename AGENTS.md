...
| **Wrapper** | `/usr/local/bin/dispatch` (forced command `DISPATCH_CONF=/etc/dispatch/storagegenie.conf /usr/local/bin/dispatch`; `run-coder` reads `coder:`/`effort:` from the packet head) | Measured 2026-09-21 by SG-071 (journal + launch artifact); supersedes the stale `/opt/storagegenie-dispatch/dispatch_coder.sh` path (legacy grok-codex-only whitelist, never reads the opencode coder) |
