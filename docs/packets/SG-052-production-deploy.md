# SG-052 — production deploy: serve the wardrobe tree live (opencode, medium)

**Dispatch params for the runner (read from this committed packet; model is the CLI default and is omitted per policy):**
coder: opencode
effort: medium

**Context and standing lines.** D69 production deploy (owner-approved 2026-09-16): the live site still serves the SG-041 image (2026-09-14); this slice rebuilds + restarts NOTHING else. No code changes, no migration (no slice since SG-041 added one — verified in-slice, see G1). **Authoring date (metadata, never a gate):** 2026-09-16. Transport: the standard job_spawn lane. Contract 0.27.0 (receipt echoes it).
...
**Standing lines:** a "pre-existing failure" claim cites the base commit + base-run command and output, or it is a new finding with a destination; compose/buildx print exit 0 even on build failure — output TEXT is the verdict source, never the exit code (SG-024); rootless docker is the runtime, project-local `BUILDX_CONFIG` route stands (SG-023/024); **AI stays OFF**: no provider calls, **$0 metered**; frozen prompts read-only. **Live-write authority (D69, owner 2026-09-16):** `docker compose up --build -d` + one idempotent re-`up` against the production stack ONLY. Any database write, any other unit/restart, any host file outside the lane → STOP.
...
**DATABASE: read-only probes (`alembic current`, per-table row counts) — NO migrate, NO seed, NO writes. Restart: the compose stack ONLY (D69). NETWORK: registry pulls for the build + app API only.**

## Why this exists

The public site serves the SG-041 image while the tree holds SG-042→SG-051. This slice makes the tree live: rebuild the image (which bakes the wardrobe UI from committed sources), restart the one loopback service, and prove the new interface answers through the existing nginx gate. Nothing else changes.

## G1 — pre-state quoted, migration-need drawn to zero (`PG-EV-08`, `PG-PR-10`)

- Stack state: `docker compose ps`, `ss -ltnp` (expect `127.0.0.1:8003` only), `curl -s http://127.0.0.1:8003/v1/health` (quote), `curl -s http://127.0.0.1:8003/` → quote the served asset hash (SG-041's was `/assets/index-khM4mS2k.js` — expect that or record what is actually served).
- DB read-only: `alembic current` (expect head `20260914_sg035_foundations`) + per-table row counts + seed-household presence check. **If `current` is not head, or any pending migration exists → STOP and report (do not migrate — unapproved).** If unexpected real rows beyond seed → STOP (`PG-IC-08`).
- Record the image id currently running (`docker compose images` / `docker ps --format`).

## G2 — build + up (the D69-authorised mutation)

- `docker compose up --build -d` (rootless; BUILDX_CONFIG route; 1800s bound) — build TEXT quoted; a build failure is read from the output text, never exit 0. No `--profile dev` (production shape: backend only).
- One idempotent second `up -d` proving no restart loop (`PG-DP-04` shape, single service).
- New image id quoted; `docker stats --no-stream` one memory sample.

## G3 — the new tree proven live, by value

- `curl -s http://127.0.0.1:8003/v1/health` → exactly `{"status":"ok","db":"ok","storage":"ok"}`.
- `curl -s http://127.0.0.1:8003/` → `200 text/html` with `<div id="root">` AND a hashed asset reference DIFFERENT from the pre-state hash (the before/after pair: pre-change captured in G1, post-change here — both raw committed, `PG-EV-09`).
- Wardrobe markers greppable in the served bundle: `sg-theme`, `Product results`, `Import assets` (quote the matching lines from the built asset or index — property, not eyeballing).
- `ss -ltnp` → 8003 on `127.0.0.1` only, no `0.0.0.0`/`::` listener for 8003/8000/5173.
- Gate intact from the outside (no credentials needed, none used): `curl -s -o /dev/null -w "%{http_code}" https://storagegenie.dynv6.net/v1/health` → `401`; `http://` → `301`. If either differs → STOP and report (nginx is outside this slice; do not touch it).
...
`{{WORKLOG_DIR}}/SG-052.log`, `{{WORKLOG_DIR}}/SG-052_report.md`, `{{WORKLOG_DIR}}/SG-052_verify.log` (before/after captures raw, gate-failure demonstration raw). First token `SG-052`; elapsed-versus-budget PER LEG with units; MODEL + effort from process arguments; spend line (`$0`); live-state ledger (image ids old→new, zero DB writes, network = build pulls + API checks); three UNCLEAR lines.
...
- Live-write authority is exactly: the compose build + up + re-up (D69). Nothing else writes. Unexpected pre-existing real data → STOP (`PG-IC-08`, either direction).
- Cross-product (`PG-IC-01`): G1 reads state; G2 the stack commands; G3 the served observables; G4 worklogs. A blanket exclusion is written with its exception in the same sentence (no new exclusion issued).
...
- No-migration-need proven (`current` == head, no pending); pre-state health + asset hash + listener state quoted.
...
- **Receipt note on the notes ref (M20-corrected block):** push the worklogs to `automation`, worktree clean (`CO-55`). No push to `storagegenie-evidence`, no `{{RECEIPT_CMD}}`. Add the note on the work HEAD (120s bound): `git notes --ref=refs/notes/storagegenie-coder-reports add -m "Dispatch-ID: SG-052 | Report: docs/worklogs/SG-052_report.md | Work-HEAD: <hash>" <WORK_HEAD>` — first line carries BOTH `Dispatch-ID:` and `Report:` (`CO-97`). Then PUSH the notes ref (300s bound): `git push origin refs/notes/storagegenie-coder-reports`. Then verify against the FETCHED ref: fetch the refspec explicitly (`<ref>:<ref>`, M21 — a bare fetch never updates a notes ref), `git notes --ref=… show <WORK_HEAD>`, and PASTE the executed output verbatim — **a receipt subsection with no pasted `show` output means the step was not executed: say so loudly instead of writing it as done.** Existing-note refusal is a STOP; final line `note=yes`; zero-exit with `note=no` is a FAIL.
