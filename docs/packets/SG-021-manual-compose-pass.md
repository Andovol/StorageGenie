...
**Stage:** D12 manual compose pass (L2 single slice) — closes ISS-1 and proves the deployment the AI phase will run in. L3 ended with the Phase 1 verdict; this slice's approval IS D12+D21. Desk `Andovol/Launcher#18` ANSWERED+CLOSED 2026-09-10: `BUILDX_CONFIG` project-local route + port `8003` recommendation; owner approvals D20 (`8003`) + D21 (this attempt). This attempt is run 1 of the `BUILDX_CONFIG` route — first-run provisional per `PG-DP-04`, a second consecutive green still owed. Prior art: SG-018's probe shape, SG-013's denial handling, SG-020's exit proof, SG-021 attempt-2 BLOCKED report (rootless green, buildx read-only + `8001` foreign — both premises now routed, neither re-litigated here).
...
Everything Phase 1 proved ran in-process (TestClient + temp DBs). Unproved on live infrastructure: the image builds with the new apt packages (SG-013's Dockerfile lines never built — `docker build` was unrunnable in-sandbox); the two decoder legs (ISS-1) go green where libzbar/tesseract exist; `compose up` survives the shared host (ports `8000`+`8001` live foreign per desk #18 CLOSED answer 2026-09-10 — NOT reclaimable, D20 remaps backend to host port `8003`, rechecked before binding); the UI serves; restart recovers to healthy. Handed tree facts (verify): `docker-compose.yml` currently publishes `8000:8000` + `5173:5173` (this slice remaps backend to `8003:8000` per D20), bind-mounts `./backend` + `./data/db`, names volume `storage_data`, requires `.env` (`env_file`), runs uvicorn `--reload`, healthchecks `GET /v1/health`, frontend `npm run dev` with `VITE_API_BASE` currently `http://localhost:8000` (this slice carries it to `http://localhost:8003` per D20), both `restart: unless-stopped`; `.gitignore` carries no `.cache/` entry (this slice adds exactly one for the builder-state dir); backend health proves DB+storage (`health.py`); runbook `README.md` names `:8000` across ~10 lines (`:40,104-140` — this slice carries each to `:8003`).
...
- G2 builder selection quoted (`buildx ls`) + activity path writable under the new route, or STOP with the persisting denial; route marked first-run provisional (`PG-DP-04`).
