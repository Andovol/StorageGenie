# SG-078 — deploy rider: drawer repair live on the public entry (opencode, medium)

**Dispatch params for the runner (read from this committed packet; model is the CLI default and is omitted per policy):**
coder: opencode
effort: medium

**Context and standing lines.** D100-approved deploy rider for SG-077 (audited 98): its work sits at `6417e53` on `automation`, NO migration ships (frontend-only — 2 files; re-verify in-slice), the production SQLite stays at head `20260917_sg068_saved_search`, but the running container still serves the SG-076-era image (bundle `index-BhyX-Hlp.js`, 303974 bytes, sha256 `256d728a34abc77dfde445ef466933751e08cc54ccc1174dfe92cf886057c744` — quoted from SG-076's measured record) WITHOUT the drawer width constraint. This slice rebuilds and brings the new image live with the STANDARD verification set. **Production restart authorized by the owner (D100) — the only authorized production mutation; everything else is read-only.** Recorded facts, verify in-slice (quoted reads):
...
- Analytics + taxonomy + saved-searches + facets + chat-gate endpoints are already live — pre-change sanity checks and post-change regression checks, never the discriminator.
...
**DATABASE: none** — no migration, no seed, no writes. **Restart: the storagegenie stack only** (one `docker compose up -d` recreate — the authorized D100 production restart).
...
- Served bundle: BEFORE quote (expected `index-BhyX-Hlp.js`, 303974 B, sha256 `256d728a…` from SG-076's measured record — a different actual is a finding, not an obstacle), then AFTER (expected DIFFERENT hash; quote the new token stylesheet too).
- **Drawer-fix discriminator (composition, stated — no browser on the box):** the new bundle served (hash change) proves the new frontend shipped HERE; SG-077's jsdom tests (`drawer.test.tsx`, panel maxWidth finite + well capped + no dead classes, fail-pre → pass-post raw) prove it renders bounded THERE. The composition is stated, not blurred, with the test reference quoted. Structural support: grep the served AFTER bundle for `440` adjacent to a max-width encoding is NOT required (minification makes it brittle) — the hash change + the committed test pair carry the claim. The owner's eyeball is the final authority (they reported the giant photo — D99).
...
- `PG-SC-09` named world where green is still wrong: a stale image (old bundle) passing health — the bundle-hash before/after + the committed test pair are the discriminators; quote the new container/image ids.
...
`{{WORKLOG_DIR}}/SG-078.log`, `SG-078_report.md`, `SG-078_verify.log` (raw curl/health/bundle/alembic outputs). First token `SG-078`; elapsed-versus-budget per leg with units; MODEL + effort from process arguments; spend (**real $** $0.000000); contract echo + source path; three UNCLEAR lines.
...
- Production surface in play: ONE recreate with NO data changes (D100, G-K2 authorized; anything beyond — extra restarts, retagging, credential fetches — STOP and report).
...
- Budget (uncalibrated per `G-A9`; SG-076 measured <60s wall): 120s ordinary · 900s build+up · 1800s overall — actual-vs-budget per leg with units.
...
- Post-bundle hash different (+ token stylesheet quoted); shipped+renders composition stated with the SG-077 test reference; analytics/taxonomy/saved-searches/facets still 200; gate 401 both sides.
...
- **Receipt note on the notes ref (M20-corrected block):** push the work to `automation`, worktree clean (`CO-55`). No push to `storagegenie-evidence`, no `{{RECEIPT_CMD}}`. Add the note on the work HEAD (120s bound): `git notes --ref=refs/notes/storagegenie-coder-reports add -m "Dispatch-ID: SG-078 | Report: docs/worklogs/SG-078_report.md | Work-HEAD: <hash>" <WORK_HEAD>` — first line carries BOTH `Dispatch-ID:` and `Report:` (`CO-97`). Then PUSH the notes ref (300s bound): `git push origin refs/notes/storagegenie-coder-reports`. Then verify against the FETCHED ref: fetch the refspec explicitly into a MAPPED local name (a default fetch never carries notes; a bare refspec fetch rewrites only FETCH_HEAD — map it), `git notes --ref=… show <WORK_HEAD>`, and PASTE the executed output verbatim — **a receipt subsection with no pasted `show` output means the step was not executed: say so loudly instead of writing it as done.** Existing-note refusal is a STOP; final line `note=yes`; zero-exit with `note=no` is a FAIL.
