# SG-004 — Complete the receipt with the evidence-prefill step (no product change)
...
Coder and correction: SG-003 ran under codex and verified everything except the receipt — its G5 omitted the evidence-prefill push that the publisher gates on (`candidate_not_remote`), root-caused at audit and filed in `docs/ratings.md` (score 82, packet-attributed). This slice completes the identical goal with that one step added. The packet is coder-neutral by design — selection lives only in the dispatch envelope, never in this file.**
...
- `{{WORKLOG_DIR}}/SG-004.log` and `{{WORKLOG_DIR}}/SG-004_report.md`, first token `SG-004`, every output path named in the report committed, three UNCLEAR lines at the end.

## G5 — Prefill the evidence ref, then auto-publish (corrected after SG-003)
...
- Only then invoke `{{RECEIPT_CMD}}` with (`SG-004`, header `contract_sync=refresh version=0.17.2`, report path, WORK_HEAD), unmodified. A repair need is a finding, never a local patch to the wrapper.
...
- Verify the artifact, not the command: the evidence ref carries a commit with the `Dispatch-ID: SG-004` trailer — quote its hash. A zero-exit publish with no such commit is a FAIL.

## Constraints

- Scope ceiling: verification only — no product behaviour change, no migration, no `.env` modification, no service restart, no LLM/OCR. No writes outside `docs/worklogs`, except the evidence-ref prefill push and the publisher's own receipt commit, which are this slice's delivery mechanism (G5).
...
- No Coder-side SSH checks: the dispatch key is absent inside the confined run (SG-003 UNCLEAR, honest and out of scope) — host paths and the prefill/publish return codes are the verification. Do not require what the confinement forbids.
...
- `docs/worklogs/SG-004.log` and `docs/worklogs/SG-004_report.md` committed; every output path in the report committed.
- `storagegenie-evidence` carries the `Dispatch-ID: SG-004` receipt commit; hash quoted.
- Prefill verified first: pre-publish `origin/storagegenie-evidence` hash quoted equal to WORK_HEAD; a post-prefill `candidate_not_remote` taken as STOP, never routed around.
