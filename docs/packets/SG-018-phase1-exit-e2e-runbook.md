# SG-018 — Phase 1 exit E2E + runbook + close-out (Codex High)

**Dispatch params for the new runner (read from this committed packet; model is the CLI default and is omitted per policy):**
coder: codex
effort: high

**Stage:** Phase 1 slice 7 of 7 — THE EXIT SLICE (plan `docs/superpowers/plans/2026-09-08-phase-1-deterministic-import.md`, D9, D10 resume). SG-012…SG-017 all landed and rated (98, 90, 96, 97, 97, 97). ISS-1 (2 decoder legs) stays carried to the manual compose pass — it is NOT re-proved here. ISS-2 is closed. This slice proves the exit condition end to end, repairs the one stale test, documents the runbook, and states the Phase 2 door. Nothing after it in this stage.
**Carries (verify, do not re-derive):** (a) stale `test_candidates.py` migration assertion (`downgrade -1` predates the SG-017 head — 3rd occurrence of the class; SG-015 set the explicit-revision pattern) → test-only repair here. Standing rule for this slice: grep the suite for head-relative downgrade assumptions and leave none stale. (b) A "pre-existing failure" claim cites the base commit + base-run command and output, or it is a new finding with a destination (standing SG-014 lever line).
...
- Verify by reading: every named command, path, route, and line reference checked against the tree; mismatches fixed, not excused. An ADR describing absent code is a finding — same rule for runbook lines.
...
- `{{WORKLOG_DIR}}/SG-018.log` and `{{WORKLOG_DIR}}/SG-018_report.md`, first token `SG-018`, every output path named in the report committed, three UNCLEAR lines at the end, elapsed-versus-budget with units. The report carries the exit-condition checklist: each blueprint:513 clause → evidence pointer (test lines + quoted values). State model/effort provenance from process arguments.
...
- Attach the receipt note to the work HEAD LAST, with no commit after it (120s bound): `git notes --ref=refs/notes/storagegenie-coder-reports add -m "Dispatch-ID: SG-018 | Report: docs/worklogs/SG-018_report.md | Work-HEAD: <hash>" <WORK_HEAD>` — the first line carries BOTH `Dispatch-ID:` and `Report:` (`CO-97`; the dispatch gate greps the ID, the runner parses the path, P3/P5). Then verify locally with `git notes --ref=refs/notes/storagegenie-coder-reports show <WORK_HEAD>` and quote the note. The RUNNER pushes the notes ref and reads it back from the remote — a note existing only locally is not a receipt.
...
- Worklog + report committed; notes ref carries the `Dispatch-ID: SG-018` + `Report:` note, quoted, dispatch result line `note=yes`.
