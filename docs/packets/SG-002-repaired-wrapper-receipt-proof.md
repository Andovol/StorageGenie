# SG-002 — Prove auto-receipt with the repaired wrapper (no product change)
...
- `{{WORKLOG_DIR}}/SG-002.log` and `{{WORKLOG_DIR}}/SG-002_report.md`, first token `SG-002`, every output path named in the report committed, three UNCLEAR lines at the end.
...
- Verify the artifact, not the command: the evidence ref carries a commit with the `Dispatch-ID: SG-002` trailer — quote its hash. A zero-exit publish with no such commit is a FAIL.
...
- `docs/worklogs/SG-002.log` and `docs/worklogs/SG-002_report.md` committed; every output path in the report committed.
- `storagegenie-evidence` carries the `Dispatch-ID: SG-002` receipt commit; hash quoted.
