...
WORK_HEAD: the exact work commit is reported in the final handoff; this report is part of that commit
...
The work commit is pushed to `automation`, then prefilled to `refs/heads/storagegenie-evidence` before the unmodified publisher invocation. The exact WORK_HEAD, prefill return code, fetched equality proof, publisher return code, and receipt trailer hash are recorded in the final handoff because the work commit hash is content-dependent on this report. The evidence artifact is required to carry trailer `Dispatch-ID: SG-004`; absence is a failure, not a vacuous pass.
...
UNCLEAR — REMAINING: The final handoff supplies the content-dependent WORK_HEAD, prefill/publisher results, receipt hash, and clean-tree proof.
