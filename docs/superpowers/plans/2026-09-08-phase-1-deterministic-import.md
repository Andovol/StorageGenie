...
**Environment capability premise (SG-013 lesson, D10 pending):** the dispatch sandbox cannot install apt packages, run sudo/docker, or reach privileged operations (verified denial 2026-09-08) — slices must prove there what runs there; anything needing system packages (`tesseract-ocr`, `libzbar0`) or the built image is pre-declared carried to the manual compose pass, whose scope therefore includes re-running the SG-013 decoder legs green plus building the image. SG-018's exit fixture asserts resilient behavior (job completes, available-extractor observations recorded, no crash), not decoder output.
