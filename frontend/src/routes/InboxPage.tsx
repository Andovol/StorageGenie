...
export function InboxPage() {
  ...
  return <div className="text-foreground" style={{ padding: 24, maxWidth: 1100 }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><h1 className="page-header text-foreground">Inbox</h1><label>Household <select value={effectiveHousehold} onChange={(event) => { setHouseholdId(event.target.value); localStorage.setItem("household_id", event.target.value); }}><option value="">Select household</option>{(households as Household[] | undefined)?.map((household) => <option key={household.id} value={household.id}>{household.name}</option>)}</select></label></div>
    ...
    <section style={{ marginTop: 28 }}><h2>Review queue</h2>{!tasks.isLoading && tasks.data?.items.length === 0 && <div role="status">No review tasks.</div>}<div style={{ display: "grid", gap: 8 }}>{tasks.data?.items.map((task) => <div key={task.id} className="bg-card border-border" style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: 12, borderStyle: "solid", borderWidth: 1, borderRadius: 8 }}><div><strong>{task.task_type}</strong> · {task.priority} · {task.status}<div className="text-muted-foreground" style={{ fontSize: 12 }}>Task {task.id} · candidate {task.subject_ref}</div></div><div style={{ display: "flex", gap: 8 }}>{task.subject_ref && <Link to={`/review/${task.subject_ref}?household_id=${effectiveHousehold}`}>Review</Link>}{task.status === "open" && <button type="button" onClick={() => resolve.mutate(task.id)} disabled={resolve.isPending}>Resolve</button>}</div></div>)}</div></section>
  </div>;
}
