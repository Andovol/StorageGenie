...
**What is in flight:** step 8 — findings-fix slice (SG-007 mypy advisory + SG-008 ALEMBIC_HEAD literal) as first self-run through the repointed forced command; then step 9 branch half (remove dead wrapper from `automation`) after sign-off D. Phase 0 exit resumes after (SG-010 drafted, untracked).
