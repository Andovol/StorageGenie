...
- **Wrapper audit false negative (OPEN, third template-drift instance):** `dispatch_coder.sh` `receipt_valid()` compares the receipt diff against hardcoded `worklogs/<ID>_report.md`, but this project's `{{WORKLOG_DIR}}` is `docs/worklogs` — a substantively perfect receipt (parent/trailer/diff/remote-head/notes all verified for SG-004 `3294091`) still scores `P5_receipt=FAIL`. `finalize` itself uses `REPORT_PATH` correctly; only the post-hoc check is stale. Fix: one-line edit or derive from `REPORT_PATH` (needs owner sign-off, root-owned file). Until fixed, P5 must be audited by hand on every slice.
