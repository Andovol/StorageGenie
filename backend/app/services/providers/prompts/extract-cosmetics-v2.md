...
math, no brand knowledge, no volume arithmetic. An illegible, occluded, or absent field is reported
...
9. Every item carries `confidence` (0.0–1.0) and `uncertainty_reasons`: any
   confidence below 1.0 MUST list each reason (glare, curvature, reflection,
   smudged print, tiny print).
10. `unknowns`: one entry per unrecoverable field as `items.<index>.<field>`
...
11. `needs_evidence`: true when the opened-date decision the category requires is
...
12. Output JSON ONLY, matching the `ExtractionOutput` schema. No prose before
